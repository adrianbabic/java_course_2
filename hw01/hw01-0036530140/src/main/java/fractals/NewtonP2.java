...
public class NewtonP2 {
	...
	public static class PosaoIzracuna  extends RecursiveAction {
		...
		public PosaoIzracuna(double reMin, double reMax, double imMin,
				...
				int m, short[] data, AtomicBoolean cancel, long requestNo, IFractalResultObserver observer, Complex[] roots, int minTracks) {
			super();
			this.reMin = reMin;
			this.reMax = reMax;
			this.imMin = imMin;
		...
		@Override
		public void compute() {
			if(minTracks < yMin - yMax) {
				computeDirect();
			}
			PosaoIzracuna posao1 = new PosaoIzracuna(reMin, reMax, imMin,
					imMax, width, height, yMin + (yMax - yMin)/2, yMax , 
					m, data, cancel, requestNo, observer, roots, minTracks);
			PosaoIzracuna posao2 = new PosaoIzracuna(reMin, reMax, imMin,
					imMax, width, height, yMin, yMin + (yMax - yMin)/2, 
					m, data, cancel, requestNo, observer, roots, minTracks);
			invokeAll(posao1, posao2);
		}
...
//			this.numThreads = numThreads;
		...
		@Override
		public synchronized void  produce(double reMin, double reMax, double imMin, double imMax,
			...
			PosaoIzracuna posao = new PosaoIzracuna(reMin, reMax, imMin, imMax, width, height, yMin, yMax,
					m, data, cancel, requestNo, observer, roots, numTracks);
			pool.invoke(posao);
